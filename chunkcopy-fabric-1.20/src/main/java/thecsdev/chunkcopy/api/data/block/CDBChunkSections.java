...
public class CDBChunkSections extends ChunkDataBlock
{
	 ...
	 * the index in {@link Chunk#getSectionArray()}, and the {@link PacketByteBuf}
	...
	public void copyData(World world, ChunkPos chunkPos)
	{
		...
		for (int y = 0; y < sectionArray.length; y++) {
			ChunkSection chunkSection = sectionArray[y];
			PacketByteBuf pbb = PacketByteBufs.create();
			chunkSection.toPacket(pbb);
			ChunkSectionData.add(new Tuple<Integer, PacketByteBuf>(y, pbb));
		}
	}
	// --------------------------------------------------
	@Override
	public void pasteData(ServerWorld world, ChunkPos chunkPos)
	{
		...
		{
			ChunkSection cs = chunk.getSection(pbb.Item1);
			cs.readDataPacket(pbb.Item2);
		}
	...
	public void readData(InputStream stream) throws IOException
	{
		...
		while(stream.available() > 0)
		{
			...
			int index = IOUtils.readVarInt(pbbStream);
			PacketByteBuf pbb = PacketByteBufs.copy(Unpooled.copiedBuffer(IOUtils.readByteArray(pbbStream)));
			pbbStream.close();
			
			ChunkSectionData.add(new Tuple<Integer, PacketByteBuf>(index, pbb));
		}
	}
	// --------------------------------------------------
	@Override
